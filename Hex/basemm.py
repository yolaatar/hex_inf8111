...
class MinimaxEngine:
    ...
    def _evaluate_state(self, state: GameStateHex) -> float:
        ...
        return self.W_DIST * dist_term + self.W_CONN * conn_term
    ...
    def _connectivity_score(self, state: GameStateHex, piece_type: str) -> float:
        ...
        (Double comptage volontaire, c'est juste un signal de structure.)
        """
        rep = state.rep
        env = rep.env
        score = 0.0
        ...
        # On pourrait /2 pour compenser le double comptage ; ce n'est pas critique
        return score
