...
class HexEngineV9:
    ...
    def choose_action(self, state: GameStateHex, time_budget: float) -> LightAction:
        ...
        deadline = start_time + max(0.05, time_budget * 0.9)
...
class MyPlayer(PlayerHex):
    ...
    def compute_action(self, current_state: GameState, remaining_time: int = 1e9, **kwargs) -> Action:
        ...
        # 4. Allocation du temps pour la recherche
        total_sec_est = float(remaining_time)
        total_sec_est = max(1.0, min(total_sec_est, 900.0))

        branching = max(1, len(possible_light_actions))
        # plus le branching est grand, plus on "divise" le temps
        adaptive_slice = min(6.0, max(0.4, math.log(branching + 1)))
        time_budget = max(0.5, min(total_sec_est / adaptive_slice, 6.0))
